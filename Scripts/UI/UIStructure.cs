...
public partial class UIStructure : Node
{
    ...
    public UIStructure UpdateCost(Dictionary<ResourceType, int> cost)
    {
        this.cost = cost;
    ...
    void HandlePurchase()
    {
        if (!int.TryParse(lineEditCount.Text, out int amount))
            return;
        int totalCostOfStructure = totalCost();

        GD.Print($"Purchased {amount} {name} for {totalCostOfStructure * amount} wood");
    }
    ...
    {
        return this.cost.Sum(cost => cost.Value);
    }
}
